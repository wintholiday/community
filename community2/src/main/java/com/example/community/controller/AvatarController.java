...
public class AvatarController {
    ...
    @PostMapping("/upload")
    public Result upload(MultipartFile image,@RequestParam("userId") int userId) throws IOException {
    ...
    public Result updateAvatar( MultipartFile avatarImage,@RequestParam("userId") int userId) {
        try {
            // 上传新头像到OSS
            ...
            log.info("头像更新完成,文件访问的url: {}", avatarUrl);
    ...
    public Result getUserAvatar(@PathVariable("userId") int userId) {
        try {
            String avatarUrl = userService.getAvatarUrlByUserId(userId);
            if (avatarUrl != null) {
