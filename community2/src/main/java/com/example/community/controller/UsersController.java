...
public class UsersController {
   ...
   /* @RequestMapping("/login")
    ...
    public Result getLike() {
        ...
        int userId = usersService.getUserIdByUsername(userDetails.getUsername());
    ...
    // 修改用户名
    @PostMapping("/change-username")
    public Result changeUsername(@RequestParam String newUsername, @RequestParam String currentUsername) {
        if (userMapper.findByUsername(newUsername) != null) {
            return Result.error("用户名已存在");
        }
        if (newUsername.equals(currentUsername)) {
            return Result.error("新用户名不能与旧用户名相同");
        }
        userMapper.updateUsername(newUsername, userMapper.findIdByUsername(currentUsername));
        return Result.success(null);
    }

    // 修改密码
    @PostMapping("/change-password")
    public Result changePassword(@RequestParam String newPassword, @RequestParam String currentUsername) {
    ...
    // 获取喜欢的文章
    @GetMapping("/like")
    public Result getLike(@RequestParam String username) {
